...
{
    const arr<float> coordinates{cast<float>(args)...};
...
Matrix rotation(const char& rotation_axis, const float& rotation_in_degrees, const int& matrix_size)
{
    const float rotation_in_radians{rotation_in_degrees * std::numbers::pi / 180};
...
Matrix four_by_four_shear(float xy, float xz, float yx, float yz, float zx, float zy)
{
